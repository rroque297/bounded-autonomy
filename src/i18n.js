...
const TRANSLATIONS = {
  ...
  en: {
    ...
    simulator: {
      ...
      sub:          'Each model configuration represents a different set of governance constraints. Running the experiment shows what happens to autonomous decisions when those constraints are absent, partial, or complete.',
    ...
    results: {
      ...
      narrativeP1:            'In this run, the baseline model produced {baseIrrev} irreversible outcome(s) and {baseViol} violation(s) across 8 scenarios — with no recovery mechanism active in any of them. Its average controllability score was {baseScore} out of 100.',
  ...
  fr: {
    ...
    results: {
      ...
      narrativeP1:     'Dans cette séquence, le modèle de base a produit {baseIrrev} résultat(s) irréversible(s) et {baseViol} violation(s) sur 8 scénarios — sans mécanisme de récupération actif dans aucun d\'eux. Son score moyen de contrôlabilité était de {baseScore} sur 100.',
  ...
  de: {
    ...
    simulator: {
      ...
      sub:          'Jede Modellkonfiguration stellt einen anderen Satz von Governance-Einschränkungen dar. Das Experiment zeigt, was mit autonomen Entscheidungen passiert, wenn diese Einschränkungen fehlen, teilweise oder vollständig vorhanden sind.',
    ...
    results: {
      ...
      narrativeP1:     'In diesem Lauf produzierte das Basismodell {baseIrrev} irreversible(s) Ergebnis(se) und {baseViol} Verstoß/Verstöße über 8 Szenarien — ohne aktiven Wiederherstellungsmechanismus in einem davon. Sein durchschnittlicher Kontrollierbarkeitsscore war {baseScore} von 100.',
...
// ── Simulator output labels + event messages
  ...
  es: {
    ...
    simulator: {
      ...
      sub:          'Cada configuración de modelo representa un conjunto diferente de restricciones de gobernanza. El experimento muestra qué ocurre con las decisiones autónomas cuando esas restricciones están ausentes, son parciales o están completas.',
    ...
    results: {
      ...
      narrativeP1:     'En esta ejecución, el modelo base produjo {baseIrrev} resultado(s) irreversible(s) y {baseViol} violación(es) en 8 escenarios — sin ningún mecanismo de recuperación activo en ninguno de ellos. Su puntuación media de controlabilidad fue {baseScore} sobre 100.',
